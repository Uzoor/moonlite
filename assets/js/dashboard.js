...
(function () {
  ...
  /* ---- forgotten password ----
     Three views share the one gate card: sign in, ask for a reset link, and
     set a new password after following that link. */
  function gateView(which) {
    const sub = $("[data-gate-sub]");
    const login = $("[data-login-form]");
    const reset = $("[data-reset-form]");
    const newpw = $("[data-newpw-form]");
    hide(login); hide(reset); hide(newpw);
    if (which === "reset") {
      show(reset);
      if (sub) sub.textContent = "We'll email you a link to set a new password.";
      if (reset) setTimeout(() => reset.email.focus(), 100);
    } else if (which === "newpw") {
      show(newpw);
      if (sub) sub.textContent = "Choose a new password for your shop.";
      if (newpw) setTimeout(() => newpw.password.focus(), 100);
    } else {
      show(login);
      if (sub) sub.textContent = "Sign in to manage your shop.";
    }
  }

  const forgotBtn = $("[data-forgot]");
  if (forgotBtn) forgotBtn.addEventListener("click", () => {
    const login = $("[data-login-form]");
    const reset = $("[data-reset-form]");
    /* carry over whatever she already typed, so she doesn't retype it */
    if (login && reset && login.email.value.trim()) reset.email.value = login.email.value.trim();
    const err = $("[data-reset-err]");
    if (err) { err.textContent = ""; err.className = "gate__err"; }
    gateView("reset");
  });

  const resetCancel = $("[data-reset-cancel]");
  if (resetCancel) resetCancel.addEventListener("click", () => gateView("login"));
  ...
  if (resetForm) resetForm.addEventListener("submit", async (e) => {
    ...
    err.className = "gate__err";
    if (!email || email.indexOf("@") === -1) { err.textContent = "Enter the email you sign in with."; return; }
    err.textContent = "";
    try {
      /* come back to this same dashboard page — this URL must also be listed
         in Supabase under Authentication → URL Configuration → Redirect URLs */
      await withBusy(btn, "Sending…", () => SB.sendRecovery(email, location.origin + location.pathname));
      err.className = "gate__ok";
      err.textContent = "Check your inbox — the link lasts about an hour.";
      hide(btn);
    } catch (ex) {
      err.textContent = (ex && ex.message) ? ex.message : "Couldn't send the email. Please try again.";
    }
  });

  const newpwForm = $("[data-newpw-form]");
  if (newpwForm) newpwForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const err = $("[data-newpw-err]");
    const btn = $("[data-newpw-btn]");
    const pw = newpwForm.password.value;
    const confirm = newpwForm.confirm.value;
    err.className = "gate__err";
    if (pw.length < 8) { err.textContent = "Use at least 8 characters."; return; }
    if (pw !== confirm) { err.textContent = "The two passwords don't match."; return; }
    err.textContent = "";
    try {
      await withBusy(btn, "Saving…", () => SB.changePassword(pw));
      newpwForm.password.value = ""; newpwForm.confirm.value = "";
      await SB.whoAmI();
      ML.toast("Password updated.");
      await enterLive();
    } catch (ex) {
      const m = (ex && ex.message) ? ex.message : "";
      /* an expired or already-used link lands here */
      if (/expired|invalid|sign in/i.test(m)) { showLoginGate("That reset link has expired. Please request a new one."); return; }
      err.textContent = m || "Couldn't save the new password. Please try again.";
    }
  });
...
`;
  ...
  applyModeUI();
  if (LIVE) {
    /* did she arrive here from a "reset your password" email? */
    const back = SB.adoptRecovery();
    if (back && back.recovery) { closeApp(); gateView("newpw"); }
    else if (back && back.error) showLoginGate(back.error);
    else if (SB.signedIn()) enterLive();
    else showLoginGate("");
  } else {
