...
(function () {
  ...
  const BASE = window.MOONLITE_CONFIG || {};
  const SEED = window.MOONLITE_SEED || [];
  const KEY_PRODUCTS = "moonlite.products.v1";
  const KEY_SETTINGS = "moonlite.settings.v1";
  const KEY_CACHE_PRODUCTS = "moonlite.cache.products.v1";
  const KEY_CACHE_SETTINGS = "moonlite.cache.settings.v1";
  ...
  /* ---------- settings (business details edited in the dashboard) -------
     config.js is the shipped default. Anything saved from the dashboard is
     kept here and layered on top, so edits preview instantly. "Export
     settings" writes a fresh config.js — that's what makes them permanent
     for visitors.                                                        */
  function readSettings() {
    const raw = store.get(KEY_SETTINGS);
    if (raw) { try { const o = JSON.parse(raw); if (o && typeof o === "object" && !Array.isArray(o)) return o; } catch (e) {} }
    return {};
  }
  function mergeCfg(base, over) {
    ...
    out.currency = Object.assign({}, base.currency || {}, over.currency || {});
    out.hours = Array.isArray(over.hours) ? over.hours.slice() : (base.hours || []).slice();
    return out;
  }
  /* CFG is mutated in place on save so every page holding a reference updates */
  const CFG = mergeCfg(BASE, readSettings());
  ...
  function saveSettings(patch) {
    const next = mergeCfg(readSettings(), patch || {});
    store.set(KEY_SETTINGS, JSON.stringify(next));
    applyCfg(mergeCfg(BASE, next));
    return next;
  }
  function resetSettings() {
    store.del(KEY_SETTINGS);
    applyCfg(mergeCfg(BASE, {}));
  }
  function baseConfig() { return BASE; }

  /* ---------- products ----------
     Two modes:
       live  — Supabase is configured. The database is the truth. We cache
               the last good copy so a dropped connection shows the shop
               as it was, never an empty page.
       local — no backend. Dashboard edits live in this browser and are
               published by exporting products.seed.js (original design).   */
  const SB = window.MLSB && window.MLSB.enabled ? window.MLSB : null;
  const MODE = SB ? "live" : "local";
  let PRODUCTS = null;      // in-memory truth once loaded
  let SOURCE = "seed";      // live | cached | seed | local
  let LIVE_SETTINGS = {};   // settings row from the database

  function readJson(key) {
    const raw = store.get(key);
    if (!raw) return null;
    try { const v = JSON.parse(raw); return v; } catch (e) { return null; }
  }

  function getProducts() {
    if (PRODUCTS) return PRODUCTS.slice();
    if (MODE === "live") {
      const cached = readJson(KEY_CACHE_PRODUCTS);
      if (Array.isArray(cached) && cached.length) { SOURCE = "cached"; return cached.slice(); }
      SOURCE = "seed";
      return SEED.slice();
    }
    const local = readJson(KEY_PRODUCTS);
    if (Array.isArray(local)) { SOURCE = "local"; return local.slice(); }
    SOURCE = "seed";
    return SEED.slice();
  }

  /* local mode only — kept so the export-a-file flow still works */
  function saveProducts(list) { store.set(KEY_PRODUCTS, JSON.stringify(list)); PRODUCTS = list.slice(); }
  function resetProducts() { store.del(KEY_PRODUCTS); PRODUCTS = null; }

  /* live mode — the dashboard calls this after a successful database write */
  function setProducts(list) {
    PRODUCTS = (list || []).slice();
    SOURCE = "live";
    store.set(KEY_CACHE_PRODUCTS, JSON.stringify(PRODUCTS));
  }

  function source() { return SOURCE; }
  function hasCache() {
    const c = readJson(MODE === "live" ? KEY_CACHE_PRODUCTS : KEY_PRODUCTS);
    return Array.isArray(c) && c.length > 0;
  }

  /* ---------- boot: pull live catalogue + settings ---------- */
  let loadPromise = null;
  function load() {
    if (loadPromise) return loadPromise;
    if (!SB) {
      PRODUCTS = getProducts();
      SOURCE = readJson(KEY_PRODUCTS) ? "local" : "seed";
      loadPromise = Promise.resolve({ mode: "local", source: SOURCE });
      return loadPromise;
    }
    loadPromise = (async () => {
      ...
      try {
        ...
        if (s && typeof s === "object") {
          LIVE_SETTINGS = s;
          store.set(KEY_CACHE_SETTINGS, JSON.stringify(s));
        }
      } catch (e) {
        const cached = readJson(KEY_CACHE_SETTINGS);
        if (cached) LIVE_SETTINGS = cached;
      }
      applyCfg(mergeCfg(BASE, LIVE_SETTINGS));

      /* then the catalogue */
      try {
        const list = await SB.listProducts();
        if (Array.isArray(list) && list.length) {
          PRODUCTS = list; SOURCE = "live";
          store.set(KEY_CACHE_PRODUCTS, JSON.stringify(list));
        } else if (Array.isArray(list)) {
          /* genuinely empty database — respect it, but don't cache emptiness */
          PRODUCTS = []; SOURCE = "live";
        }
      } catch (e) {
        const cached = readJson(KEY_CACHE_PRODUCTS);
        if (Array.isArray(cached) && cached.length) { PRODUCTS = cached; SOURCE = "cached"; }
        else { PRODUCTS = SEED.slice(); SOURCE = "seed"; }
      }
      return { mode: "live", source: SOURCE };
    })();
    return loadPromise;
  }

  /* live mode — write business details to the database */
  async function saveSettingsRemote(patch) {
    if (!SB) return saveSettings(patch);
    const next = mergeCfg(LIVE_SETTINGS, patch || {});
    await SB.pushSettings(next);
    LIVE_SETTINGS = next;
    store.set(KEY_CACHE_SETTINGS, JSON.stringify(next));
    applyCfg(mergeCfg(BASE, next));
    return next;
  }
  function liveSettings() { return LIVE_SETTINGS; }
  ...
  /* ---------- currency ---------- */
  function curr() { return CFG.currency || { code: "NGN", symbol: "₦", locale: "en-NG" }; }
  function money(n) {
    const cur = curr();
    try {
  ...
  function wireChrome() {
    ...
    if (chromeWired) return;   // document/window listeners attach once only
    chromeWired = true;
  ...
  /* ---------- expose ---------- */
  window.ML = {
    CFG, money, sha256, toast, mode: MODE,
